...
import 'package:hello/seller/returnpolicy.dart';
...
class _BuyPageState extends State<BuyPage> {
  ...
  Future<void> _fetchBuyProducts() async {
    final response = await http.get(
      Uri.parse("http://192.168.205.252/flutter_api/get_sell_products.php"),
    );
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      if (data['success']) {
        setState(() {
          _buyProducts = List<Map<String, dynamic>>.from(data['products']);
        });
      }
    }
  }
        ...
        title: Row(
          children: [
            ...
            IconButton(
              ...
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => Like_buy.LikePage())),
            ),
            IconButton(
              icon: Icon(Icons.shopping_cart_outlined, color: textColor),
              onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => Cart_buy.ShoppingBag())),
            ),
          ],
        ),
      ),
          ...
          children: [
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Text('Buy', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold,color: textColor)),
                  SizedBox(height: 10),
                  Expanded(
                    child: _buyProducts.isEmpty
                      ...
                      itemBuilder: (context, index) {
                        final product = _buyProducts[index];
                        final List<dynamic> images = product['product_images'] ?? [];
  ...
  Widget _buildProductItem({required String imageUrl, required String title, required String price, required Color textColor}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        AspectRatio(
